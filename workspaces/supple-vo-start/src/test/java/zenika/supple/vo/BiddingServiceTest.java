...
public class BiddingServiceTest {
    ...
    private final Bid
            lowBid          = Bid.on(theAuction).at(        1, "EUR"),
            midBidInEuros   = Bid.on(theAuction).at(      100, "EUR"),
            midBidInDollars = Bid.on(theAuction).at(      100, "USD"),
            highBid         = Bid.on(theAuction).at(1_000_000, "EUR");
    ...
    @Test public void should_be_winning_when_no_other_bids() {
        whenThereAreNoBids();
            assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(WINNING);
    }
    ...
    @Test public void should_be_losing_when_bid_is_lower() {
        whenHighestBidIs(highBid);
            assertThat(outcomeOfPlacingMidBidInEuros()).isEqualTo(LOSING);
    }
    ...
    private BidOutcome outcomeOfPlacingMidBidInEuros() {
